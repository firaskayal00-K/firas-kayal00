...
## Attack-and-block (a threat bounces off a shield)
For "it won't charge you" / "blocked" moments: the protected thing (CSS credit card) hovers (y yoyo); shield slams onto it; then a RED threat token (e.g. a $ coin) flies IN from off-screen (power2.in), hits the shield edge, spark-burst + shake, and DEFLECTS away (x/y off opposite corner, rotation 180, fade). Threat uses immediateRender:false. Far more engaging than a static shield.

## Banned-globe -> VPN tunnel
"Not available in your country": wireframe globe (SVG circle+ellipses+lines) sways gently (rotation -8->8 sine); a red BAN sign (circle+slash SVG) stamps over it (scale 2.6->1 power4.out) while the globe grayscales, + shake. On "VPN": ban scales up and fades with a burst, globe revives (grayscale 0), a GREEN checkmark (#22c55e, stroke-dashoffset draw 0.35s + scale-punch) lands over the globe with a soft green radial glow bloom, and a GREEN VPN pill slams under. (User rejected a dashed tunnel arc here — 'solved' reads as green check, not more lines. General dashoffset gotcha: an offset larger than the path length WRAPS and shows the dash pattern early; keep such elements opacity:0 until draw time.)

## Real YouTube video card (CTA to a longer video)
Instead of a generic play button: fetch the ACTUAL thumbnail (WebSearch the title -> video id -> i.ytimg.com/vi/<id>/maxresdefault.jpg), compose an IG-style video card: rounded thumbnail (860x484) + red play button centered (pulse loop) + bold title + gray "54K views · 1 month ago" meta. Slam in on "YouTube", scale-punch on "watch that".


## Physics confetti (payoff bursts)
Single-burst radial confetti reads cheap. Real feel: ~26 mixed pieces (rects + circles, 5-color palette incl. green), each with TWO keyframes — launch UP-and-out (y -140..-420, x ±420, rotation, power2.out, 0.38-0.6s) then FALL with gravity (y +240..+520, extra x drift and spin, power1.in, 0.7-1.1s) — opacity fading over the whole 1.3-1.7s life, per-piece random scale 0.6-1.2 and start-delay 0-0.12s. immediateRender:false on all.
