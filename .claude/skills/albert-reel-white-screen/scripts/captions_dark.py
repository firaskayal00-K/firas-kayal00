...
for i in range(n):
    ...
    if word:
        ...
        ds.text((x, CY+4), word, font=F, fill=(255, 255, 255, 170))
        ds.text((x, CY), word, font=F, fill=(255, 255, 255, 170))
        img = Image.alpha_composite(img, sh.filter(ImageFilter.GaussianBlur(9)))
        ImageDraw.Draw(img).text((x, CY), word, font=F, fill=(20, 20, 20, 255))   # clean white on top
