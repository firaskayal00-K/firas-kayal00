...
| Trusting silence_keep + interleaved-take EDL ends | When raw retakes overlap in the transcript (Scribe interleaves the timings), the chosen EDL ends are already tight — running `silence_keep.py` on top then clips word tails ("dol-", "subs-", "ev-"). On long-form: re-transcribe after EVERY cutting pass and scan for hyphen-truncated words; if takes were hand-picked tightly, skip the silence pass (it saves ~2s and risks 4 clipped words) and instead extend the raw EDL ends +0.3s and remove residual stutters with a second small EDL pass on the cut timeline. |
| Assuming you must regenerate all B-roll | Check `broll/` first — an earlier session on the same video may have identity B-roll already downloaded that only needs re-slicing; regenerate only what the new color theme invalidates (e.g. green-LED datacenter → blue). |
